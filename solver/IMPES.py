...
well.Pwf            = np.zeros((len(well.x),nmax+1))
well.BlockPress     = np.zeros((len(well.x),nmax+1))
well.qwf_o              = np.zeros((len(well.x),nmax+1))
well.qwf_w              = np.zeros((len(well.x),nmax+1))


while (t[k] < numerical.tfinal):                # non dimensional time marching
    if (k == nmax/3 or k == nmax/1.5 or k == nmax/1 or k == nmax):
        print(k, t[k], Sw, P)

    P_old = np.copy(P)  # Placeholdering the old array
    Sw_old = np.copy(Sw)  # Placeholdering the old array
    P[numerical.D == 0.0] = 7000

    P_old = np.copy(P)

    # Calculating the arrays
    Tw, To, T, d11, d12, d21, d22, D, Q, G, Pc, Pw = myarrays(fluid, reservoir, petro, numerical, IC, BC, P, Sw)
    # updating the wells
    well, Qw, Qo, Jw, Jo = updatewells(reservoir, fluid, numerical, petro, P, Sw, well)
    J = -d22 @ (spdiaginv(d12) @ Jw) + Jo
    Q = Q + -d22 @ (spdiaginv(d12) @ Qw) + Qo + reservoir.Pwf * J @ np.ones((numerical.N, 1)) # Pwf = 5300 psi

    for i in range(0, len(well.x)):
        kblock = well.block[i][0]
        krw, kro = rel_perm(petro, Sw[kblock, 0])
        M = (kro * fluid.muw[kblock, 0] * fluid.Bw[kblock, 0]) / (krw * fluid.muo[kblock, 0] * fluid.Bo[kblock, 0])
        well.fw[i, k] = 1 / (1 + M)
        ...
        if well.type[i][0] == 1:  # 1 for rate
            if well.constraint[i][0] > 0.0:  # injector well
                well.fw[i, k] = 1.0
            well.Pwf[i, k] = P[well.block[i],0] + well.constraint[i][0] / (well.Jovectime[i,k] + well.Jwvectime[i,k])
            well.BlockPress[i, k] = P[well.block[i],0]
            well.qwf_w[i, k] = well.constraint[i][0]
            well.qwf_o[i, k] = 0.0

        elif (well.type[i][0] == 2):  # 2 for BHP
            well.Pwf[i, k] = well.constraint[i][0]
            well.BlockPress[i, k] = P[well.block[i],0]
            well.qwf_o[i, k] = well.Jovectime[i,k] * (P[well.block[i],0] - well.constraint[i][0])
            well.qwf_w[i, k] = well.Jwvectime[i,k] * (P[well.block[i],0] - well.constraint[i][0])

        #well.Qo[i, k] = well.Jovectime[i, k] * (1-well.fw[i,k])
        #well.Qw[i, k] = well.Jwvectime[i, k] * (well.fw[i,k])
    error = 1
    tol = 1E-05

    if numerical.method == 'IMPES':
        IM = T + J + D  # implicit part coefficient Eq
        EX = D @ P_old + Q + G  # explicit part or RHS Eq
        P = np.transpose(gmres(IM, EX, tol=1E-08, restart=10, maxiter = 1000))  # solving IM*P = EX or Ax=B
    ...
    well.Qwf[:, k] = (Qw[well.block, 0].toarray())[:, 0] + (Qo[well.block, 0].toarray())[:, 0] + (
                well.Jwvec[:, 0] + well.Jovec[:, 0]) * reservoir.Pwf
...
Sw_plot[np.argwhere(reservoir.permx < 0.0000001)] = np.nan
...
initial_plot(reservoir, numerical, P_plot[:, int(nmax/3)], t[int(nmax/3)], 'P')
initial_plot(reservoir,numerical,P_plot[:,int(nmax/1.5)],t[int(nmax/1.5)],'P')
initial_plot(reservoir, numerical, P_plot[:, nmax], t[nmax], 'P')

initial_plot(reservoir, numerical, Sw_plot[:, 0], t[0], 'Sw')
initial_plot(reservoir, numerical, Sw_plot[:, int(nmax/3)], t[int(nmax/3)], 'Sw')
initial_plot(reservoir, numerical, Sw_plot[:, int(nmax/1.5)], t[int(nmax/1.5)],'Sw')
