...
from input_file_2D_3 import inputfile
...
while (t[k] < numerical.tfinal): #non dimensional time marching    
    ...
    while error > tol:
        ...
        if numerical.method == 'IMPES':
            ...
            P = np.transpose(gmres(IM,EX, tol=1e-08, restart=10, maxiter=154)) #solving IM*P = EX or Ax=B
            #print(P)
    ...
    Sw[np.argwhere(Sw > 1.0)] = 1.0
    P[np.argwhere(numerical.D == 0.0)] = 1000
    Sw[np.argwhere(IC.Sw < petro.Swr)] = 0.18
