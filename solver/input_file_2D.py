...
from cap_press import cap_press, cap_press_init
...
def inputfile(fluid,reservoir,petro,numerical,BC,IC,well):
    ...
    numerical.dt     = 1                        #float(input('Enter timestep: ', ))  #time step (days)
    numerical.tfinal = 300                       #final time [days]
    ...
    # Fluid parameters
    fluid.muw = 0.52 * np.ones((numerical.N, 1))                    #water viscosity [centipoise]
    fluid.rhw = 62.4 * np.ones((numerical.N, 1))                 #water density [lb/ft3]
    ...
    fluid.muo = 0.92 * np.ones((numerical.N, 1))                    #fluid viscosity [centipoise]
    fluid.API = 50                                                  #fluid API gravity
    fluid.rho = 48.65 * np.ones((numerical.N, 1))                 #oil density [lb/ft3]
    fluid.Bo  = 1.00 * np.ones((numerical.N,1))                  #formation volume factor of oil [rb/stb]
    fluid.co  = 5.0E-6                                              #oil compressibility: oil fluid [1/psi]
    fluid.ct  = 8.9E-6                                             #total compressibility: rock + fluid [1/psi]

    fluid.sg = 0.65                      # SG of the gas
    fluid.Rs = 923                       # GOR of the gas @ BP
    fluid.rhwsc   = 62.4                    # density of the water [lbm/ft^3]
    fluid.rhosc = 48.65                    # density of the stock tank oil [lbm/ft^3]
    fluid.BP     = 5000                     # bubble point pressure [psi] @ reference pressure
    fluid.Bo_ref   = 1.00                # formation volume factor at reference pressure [rb/stb]
    fluid.Bw_ref   = 1.00                # formation volume factor at reference pressure [rb/stb]
    fluid.rhw_ref = 62.4                     #water density [lb/ft3] at reference pressure
    fluid.rho_ref = 48.65                     #oil density [lb/ft3] at reference pressure
    fluid.muo_ref = 0.92                   # viscosity constant @ BP
    ...
    petro.Pe = 0.7071           # capillary entry pressure [psi]

    # Reservoir parameters
    reservoir.L  = np.sum(dx[0,:])                             #length of the reservoir [feet]
    reservoir.h  = np.reshape(thickness, (numerical.N,1))  #22 * np.ones((numerical.N,1))       # height of the reservoir [feet]
    reservoir.h[reservoir.h == 0.0] = 1E-16
    reservoir.H = 20                                        #height of the reservoir [feet]
    ...
    reservoir.permx[reservoir.permx <= 1E-6] = 1E-20
    ...
    reservoir.Dref   = 9290                                             #reference depth [feet]
    reservoir.phi_ref   = 0.214            #reference porosity [-]
    reservoir.perm_ref   = 279.6            #reference perm [mD]
    reservoir.alpha  = 0.0 * np.pi/6.0        #dip angle [in radians]
    reservoir.cfr    = 5.9E-06               #formation of rock compressibility
    ...
    reservoir.phi[reservoir.phi==0] = 1E-20
    #reservoir.flag   = np.reshape(flag,(numerical.N,1))           # Flag

    #Defining numerical parameters for discretized solution
    numerical.dx1 = np.reshape(dx[1,:], (numerical.Nx, 1))      #block thickness in x vector
    numerical.dy1 = np.reshape(dy[:,1], (numerical.Ny, 1))      #block thickness in y vector
    ...
    numerical.D = numerical.D + 0.5 * reservoir.h

    # Well Grid Location
    well.x = [numerical.Xc[2,8], numerical.Xc[3,3], numerical.Xc[1,2]]
    well.y = [numerical.Yc[2,8], numerical.Yc[3,3], numerical.Yc[1,2]]

    well.x = [[well.x[0]], [well.x[1]], [well.x[2]]]                    # well location in x-direction
    well.y = [[well.y[0]], [well.y[1]], [well.y[2]]]                    # well location in y-direction
    well.type = [[2], [1], [2]]                                         # 1 for rate, 2 for BHP
    well.constraint = [[5300], [250], [5300]]                           #rate = scf/day (+ for injector); BHP = psi (always +ve)
    well.rw = [[0.25], [0.25], [0.25]]                                   # well radius, ft
    ...
    print('well.xloc :', well.x)
    print('well.yloc :', well.y)

    # Boundary Conditions
    BC.type = [['Neumann'], ['Neumann'], ['Neumann'], ['Neumann']]  # type of BC: left, right, bottom, top
    BC.value = [[0.0], [0.0], [0.0], [0.0]]  # value of the boundary condition: psi or ft^3/day
    BC.depth = [[reservoir.Dref],[reservoir.Dref],[reservoir.Dref],[reservoir.Dref]]
    ...
    IC.Sw = 0.5 * np.ones((numerical.N, 1))  # Initial saturation
    ...
    while error > tol:
        ...
        #IC.Pw = (reservoir.Pref - petro.Pe) + (fluid.rhw / 144) * (numerical.D - reservoir.Dref)
        #IC.P = reservoir.Pref + (fluid.rhosc / 144) * (numerical.D - reservoir.Dref)
        IC.Pc, Pcprime = cap_press_init(petro, IC.Sw, cap=cap)
        IC.P = IC.P
        #IC.Pc = (IC.P - IC.Pw)
        IC.Pw = IC.P - IC.Pc
        IC.Sw = IC.Sw
...
Sw = IC.Sw
Sw_hyst = IC.Sw
Pc, Pcprime = cap_press(petro, Sw, Sw_hyst)
print(Pc)
print(Pcprime)
print(IC.P)
for i in range(0, numerical.Nx):
    for j in range(0,numerical.Nx):
