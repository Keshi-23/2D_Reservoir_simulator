...
from myarrays_3 import myarrays
...
def inputfile(fluid,reservoir,petro,numerical,BC,IC,well):
    ...
    petro.Sor  = 0.10            #residual oil saturation
    ...
    reservoir.h  = 20                  # np.reshape(thickness, (numerical.N,1)) #height of the reservoir [feet]
    ...
    while error > tol:
        ...
        IC.Pw = IC.P - IC.Pc
        #IC.Sw = petro.Swr + (1.0 - petro.Swr) * (abs(IC.Pc) / petro.Pe) ** (-petro.lamda)
        IC.Sw = IC.Sw
        fluid_properties(reservoir, fluid, IC.P, IC.Pw)
