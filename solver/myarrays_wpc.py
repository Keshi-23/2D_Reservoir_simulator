...
def myarrays(fluid,reservoir,petro,numerical,BC,P,Sw,Sw_hyst):
    ...
    for l in range(0,numerical.N):
        Pc[l,0],Pcprime = cap_press(petro,Sw[l,0],Sw_hyst[l,0]) 
        Pw[l,0]  = P[l,0] - Pc [l,0]
        ...
        if (l+1) % numerical.Nx != 1:  #not on left boundary
            ...
            if 'Neumann' in BC.type[0]:
                ...
                Q[l,0] = Q[l,0] + 2 * Thalf(l,l,'x',fluid,reservoir,numerical,P,Pw,Pc,Sw)* (BC.value[0][0] - (fluid.rho/144.0)*numerical.D[l,0] )* 6.33E-3 #with gravity

            elif 'Dirichlet' in BC.type[0]:
                Tw[l,l], To[l,l]  = To[l,l] + 2 * Thalf(l,l,'x',fluid,reservoir,petro,numerical,P,Pw,Pc,Sw)
                # None
        ...
        if (l+1) % numerical.Nx != 0:  #not on right boundary
            ...
            if 'Neumann' in BC.type[1]:
                ...
                #None
                T[l,l] = T[l,l] + 2 * Thalf(l, l, 'y', fluid, reservoir, numerical,P,Pw,Pc,Sw)

        if int(l / numerical.Nx) > 0:  #not bottom boundary
             Twhalf, Tohalf = Thalf(l,l-numerical.Nx,'y',fluid,reservoir,petro,numerical,P,Pc,Sw)
            ...
            if 'Neumann' in BC.type[2]:
                #None
                Q[l, 0] = Q[l, 0] + 2 * Thalf(l, l, 'y', fluid, reservoir, numerical,P,Pw,Pc,Sw) * (BC.value[2][0] \
                                - (fluid.rho / 144.0) * numerical.D[l, 0]) * 6.33E-3  # with gravity
        ...
        if int(l / numerical.Nx) < numerical.Ny - 1:  #not top boundary
            ...
            if 'Neumann' in BC.type[3]:
                ...
                Q[l, 0] = Q[l, 0] + 2 * Thalf(l, l, 'y', fluid, reservoir, numerical,P,Pw,Pc[l,0],Sw) * (BC.value[3][0] - (fluid.rho / 144.0) * numerical.D[l, 0]) * 6.33E-3  # with gravity
        ...
        Vb = numerical.dx[l,0] * numerical.dy[l,0] * reservoir.h[l,0]
    ...
    G = -d22 @ (spdiaginv(d12) @ (Tw @ (P - Pw))) - d22 @ (spdiaginv(d12) @ (Tw @ numerical.D)) * fluid.rhw /144.0 + (fluid.rho/144.0 * To) @ numerical.D
